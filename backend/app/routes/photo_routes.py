...
def recognize_board():
    if 'image' not in request.files:
        return jsonify({'error': 'Ключ "image" не найден в запросе (FormData)'}), 400
    ...
    if not allowed_file(image.filename):
        return jsonify({'error': 'Разрешены только форматы JPG, PNG и HEIC'}), 400
...
def correct_position():
    ...
    if not data.get('fen'):
        return jsonify({'error': 'FEN обязателен'}), 400
...
def analyze_confirmed():
    ...
    if not fen or len(fen) < 15:
        return jsonify({'error': 'Некорректная позиция для анализа'}), 400
