...
@roadmap_bp.route('/tasks/<int:task_id>/content', methods=['GET'])
@jwt_required()
@limiter.limit("30 per minute")
def get_task_content(task_id):
    user_id = get_jwt_identity()
    result, status = RoadmapService.get_task_content(user_id, task_id)
    return jsonify(result), status


@roadmap_bp.route('/tasks/<int:task_id>/quiz', methods=['POST'])
@jwt_required()
@limiter.limit("30 per minute")
def submit_task_quiz(task_id):
    user_id = get_jwt_identity()
    data = request.get_json(silent=True) or {}
    answers = data.get('answers', [])
    result, status = RoadmapService.submit_task_quiz(user_id, task_id, answers)
    return jsonify(result), status
